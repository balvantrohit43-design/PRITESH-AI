...
// ========== BOT STATE ==========
let last10Numbers = [];
let predictionsMap = {};           // period -> prediction object
let resultsHistory = [];
let totalTrades = 0;
let wins = 0;
...
async function update() {
    ...
    if (lastProcessedPeriod !== current.period) {
        if (predictionsMap[current.period]) {
            ...
            console.log(`[INFO] Period ${current.period} has no prediction (first run)`);
        }
...
app.get('/trade', (req, res) => {
    ...
    // Get the latest (highest period) prediction
    const periods = Object.keys(predictionsMap).sort((a, b) => parseInt(b) - parseInt(a));
